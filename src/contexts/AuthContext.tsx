...
interface AuthContextType {
  ...
  user: User | null;
  role: AppRole | null;
  profile: { name: string; department: string | null; avatar_url: string | null } | null;
  loading: boolean;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUserMeta = async (userId: string) => {
    const [roleRes, profileRes] = await Promise.all([
      ...
      supabase.from("profiles").select("name, department, avatar_url").eq("user_id", userId).single(),
    ]);
